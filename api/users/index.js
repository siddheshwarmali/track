...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === 'POST') {
      ...
      log(sess.userId, 'user-manager', 'create-user', `Created user: ${userId} (Role: ${role})`);
    ...
    if (req.method === 'PUT') {
      ...
      log(sess.userId, 'user-manager', 'update-user', `Updated user: ${userId}`);
      return json(res, 200, { ok: true, user: all.users[userId] });
    }
    ...
    if (req.method === 'DELETE') {
      ...
      log(sess.userId, 'user-manager', 'delete-user', `Deleted user: ${userId}`);
