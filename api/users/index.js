...
function canManageUsers(me){
  if(!me) return false;
  if(me.role === 'admin') return true;
  return !!(me.permissions && me.permissions.userManager);
}
...
module.exports = async (req, res) => {
  try {
    ...
    const me = all.users[sess.userId] || { role: sess.role || 'viewer', permissions:{} };

    if(!canManageUsers(me)) return json(res, 403, { error:'Forbidden: User Manager access required' });

    if(req.method === 'GET'){
      const users = Object.values(all.users).map(u=>({
        userId: u.userId,
        role: u.role,
