...
function cfg(){ return { owner: must('GITHUB_OWNER'), repo: must('GITHUB_REPO'), branch: process.env.GITHUB_BRANCH||'main', token: must('GITHUB_TOKEN'), apiVersion: process.env.GITHUB_API_VERSION||'2022-11-28' }; }
function headers(c){ return { 'Accept':'application/vnd.github+json', 'Authorization':`Bearer ${c.token}`, 'X-GitHub-Api-Version': c.apiVersion }; }
...
async function ghPutFile(path, textContent, message, sha){
  ...
  const body={ message, content: Buffer.from(textContent,'utf-8').toString('base64'), branch:c.branch };
  if(sha) body.sha=sha;
...
async function ghPutFileRetry(path, textContent, message, sha){
  try{ return await ghPutFile(path, textContent, message, sha); }
  catch(e){ if(String(e).includes('409')){ const f=await ghGetFile(path); return await ghPutFile(path, textContent, message, f.exists?f.sha:null);} throw e; }
}
