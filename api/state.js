...
<head>
    ...
    <title>Executive Dashboard - Build</title>
<!-- COPILOT_BUGFIX_STAMP: 2026-01-23 11:38:56 IST | responsive-header + actions-menu-scroll + publish-toggle + exportReport-blob-unique-cachebust -->
...
<script>
 ...
 if(eb){ const perms = me.permissions || {}; eb.classList.toggle('hidden', !(role==='admin' || perms.executiveBoard)); }
  ...
  <script src="/assets/js/state-client.js" defer></script>
  <script src="/assets/js/build-extras.js" defer></script>

<style id="tw-responsive-overrides">
  /* Copilot bugfix overrides */
  @media (max-width: 640px){
    .tw-sticky-header-fix{ position: sticky; top: 0; }
    .tw-sticky-header-fix *{ max-width: 100%; }

    /* Actions bottom-sheet should not overflow */
    #ws-actions-menu{ max-height: 85vh; }

    /* Fullscreen heavy modals */
    #modal-l3 > div,
    #modal-us-details > div,
    #modal-discipline > div,
    #modal-mapping > div{
    ...
    /* Stack report controls */
    .no-print.mobile-stack-actions{ flex-direction: column !important; align-items: stretch !important; }
    .no-print.mobile-stack-actions > button{ width: 100% !important; justify-content: center !important; }
  }
</style>


<style id="tw-auto-scale-safe">
/* Auto scale text & layout safely */
html { font-size: clamp(14px, 1vw, 16px); }
body { overflow-x: hidden; }

/* Make tables & charts responsive */
table { width: 100%; table-layout: fixed; }
canvas { max-width: 100% !important; height: auto !important; }
</style>

<style id="tw-header-responsive-safe">
/* Mobile header layout */
@media (max-width: 640px) {
  .tw-sticky-header-fix > div {
    display: flex !important;
    flex-direction: column !important;
    gap: 0.5rem !important;
  }

  /* Workspace + actions wrap nicely */
  #ws-select { width: 100% !important; }
  #ws-actions-btn { width: 100%; justify-content: center; }
}
</style>


<style id="tw-zoom-fix">
/* === GLOBAL ZOOM & FIT FIX === */
*, *::before, *::after { box-sizing: border-box; }

html {
  font-size: clamp(14px, 0.95vw + 0.2rem, 16px);
}
...
<!-- === MOBILE-FIRST RESPONSIVE UI FIX (PRODUCTION SAFE) === -->
<style id="tw-mobile-responsive-final">
...
/* ---------- TABLES ---------- */
/* Tables should NEVER squeeze on mobile */
@media (max-width: 768px) {
  ...
  table {
    min-width: 640px; /* forces scroll instead of vertical text */
  }
...
<!-- === /MOBILE-FIRST RESPONSIVE UI FIX === -->


<!-- === BUTTON RESPONSIVENESS FIX (MOBILE + WEB SAFE) === -->
<style id="tw-button-responsive-fix">
...
<!-- === MOBILE UI POLISH: BUTTONS + TITLES (FINAL) === -->
<style id="tw-mobile-polish">

/* ---------- TITLES (User Story / Bugs) ---------- */
@media (max-width: 768px) {
  #section-user-stories h3,
  #section-bugs h3 {
...
@media (max-width: 768px) {
  ...
  /* Action groups: wrap instead of stack */
  .flex.space-x-2,
  .flex.space-x-3,
  .flex.gap-2 {
  ...
  /* Only critical action buttons full-width */
  .mobile-primary {
    width: 100% !important;
  }
...
<!-- ===== Header (Run-style layout + Build workspace actions) ===== -->
<div class="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-slate-200 tw-sticky-header-fix">
  <div class="max-w-7xl mx-auto px-3 sm:px-4 py-3 sm:py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div class="min-w-0">
      <div class="text-lg font-extrabold truncate">Executive Dashboard — Build</div>
      <div class="text-xs text-slate-500 truncate">
        Workspace: <span id="dashPill" class="font-mono">—</span>
      </div>
    </div>

    <div class="w-full sm:w-auto grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:gap-2 sm:justify-end">
      <select id="ws-select" class="ml-0 sm:ml-1 col-span-2 sm:col-auto w-full sm:w-56 px-3 py-2 border border-gray-300 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">Loading…</option>
                    </select>
                    <span id="ws-saved-indicator" class="text-xs text-gray-500 ml-2">Not saved</span>

      <!-- Publish/Private pill (optional, reused from Run ids) -->
  ...
  <button id="ws-actions-btn" type="button"
    ...
    <span class="text-base">⚙️</span><span>Actions</span>
  ...
  <div id="ws-actions-backdrop" class="hidden fixed inset-0 bg-slate-950/30 z-40" aria-hidden="true"></div>
  <div id="ws-actions-menu" class="hidden z-50 fixed pb-[env(safe-area-inset-bottom)] inset-x-4 bottom-4 sm:inset-auto sm:absolute sm:right-0 sm:top-full sm:mt-2 w-[calc(100%-2rem)] sm:w-64 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[85vh] sm:max-h-none" role="menu" aria-label="Workspace actions">
    <div class="sm:hidden px-4 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between"><div class="text-sm font-semibold text-slate-800">Workspace Actions</div><button type="button" class="text-slate-500 hover:text-slate-800" data-ws-close aria-label="Close">✕</button></div>
    <div class="p-2 overflow-y-auto flex-1 custom-scroll">
      ...
      <button type="button" class="w-full text-left px-3 py-3 sm:py-2 rounded-xl text-sm hover:bg-slate-50 active:bg-slate-100" data-ws-action="exportReport" role="menuitem">Export Report (JPG)</button>
      ...
      <button type="button" class="w-full text-left px-3 py-3 sm:py-2 rounded-xl text-sm hover:bg-slate-50 active:bg-slate-100" data-ws-action="publish" role="menuitem" id="ws-action-publish">Publish</button>
      <button type="button" class="w-full text-left px-3 py-3 sm:py-2 rounded-xl text-sm hover:bg-slate-50 active:bg-slate-100" data-ws-action="unpublish" role="menuitem" id="ws-action-unpublish">Unpublish</button>
    </div>
...
<style>
  /* Tighten content gap under header */
  body > div.sticky + .max-w-7xl { padding-top: 0 !important; }
</style>
...
<div id="tw-app-loader" class="fixed inset-0 z-[99999] bg-slate-950/50 backdrop-blur-sm hidden">
  <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[min(520px,92vw)] bg-white rounded-2xl shadow-2xl border border-slate-200 p-5">
    <div class="flex items-center gap-3">
      ...
      <div>
        <div class="text-sm font-semibold text-slate-900" id="tw-loader-title">Loading dashboard…</div>
    ...
    <div class="mt-4">
      ...
      <div class="mt-2 text-[11px] text-slate-500" id="tw-loader-hint">Tip: Use menu → Workspace → Save to persist to GitHub.</div>
    </div>
  </div>
</div>


    <div class="max-w-7xl mx-auto px-6 pb-6 pt-3 min-h-screen">
        <!-- Header -->
...
<!-- Main Card -->
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div id="content-build" class="animate-fade-in">
                ...
                <!-- Report Controls -->
                <div class="flex flex-col sm:flex-row sm:justify-end gap-3 mb-4 no-print mobile-stack-actions">
    ...
    <script>
        ...
        async function exportReport() {
            // Close menus/backdrops so they don't get captured
            try {
                document.getElementById('ws-actions-menu')?.classList.add('hidden');
                document.getElementById('ws-actions-backdrop')?.classList.add('hidden');
                document.getElementById('tw-menu')?.classList.add('hidden');
            } catch (e) {}

            const btn = document.querySelector('[data-ws-action="exportReport"]') || document.querySelector('button[onclick="exportReport()"]');
            const originalContent = btn ? btn.innerHTML : null;

            try {
                if (btn) {
                    btn.innerHTML = `<i data-lucide="loader-2" class="w-4 h-4 inline-block align-middle mr-2 animate-spin"></i><span>Generating…</span>`;
                ...
                if (!element) throw new Error('Build content not found');
                ...
                const canvas = await html2canvas(element, {
                    ...
                    ignoreElements: (el) => {
                        ...
                        if (id.includes('modal') || id.includes('toast') || id.includes('menu') || id.includes('backdrop') || id.includes('loader') || id.includes('overlay')) return true;
                        return false;
                    }
                });

                // Use UNIQUE filename to avoid Android/iOS caching/overwriting showing old image
                const pad = (n) => String(n).padStart(2,'0');
                const now = new Date();
                const stamp = `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
                const filename = `Executive_Report_${stamp}.jpg`;
                ...
                await new Promise((resolve, reject) => {
                    canvas.toBlob((blob) => {
                        try {
                            ...
                            a.click();
                            a.remove();
                            // revoke later for Safari reliability
...
function renderBlockers() {
        ...
        function openMilestoneModal(idx = null) {
            ...
            if(isEdit) { const m = milestones[idx]; document.getElementById('ms-phase').value = m.phase; document.getElementById('ms-start').value = m.startDate; document.getElementById('ms-end').value = m.endDate; document.getElementById('ms-act-start').value = m.actualStartDate; document.getElementById('ms-act-end').value = m.actualEndDate; document.getElementById('ms-owner').value = m.owner || ''; document.getElementById('ms-remark').value = m.remark; } 
            else { ['ms-phase','ms-start','ms-end','ms-act-start','ms-act-end','ms-owner','ms-remark'].forEach(id => document.getElementById(id).value = ''); }
            document.getElementById('modal-milestone').classList.add('open');
        }
        function saveMilestone() { const idx = document.getElementById('ms-index').value; const item = { phase: document.getElementById('ms-phase').value, startDate: document.getElementById('ms-start').value, endDate: document.getElementById('ms-end').value, actualStartDate: document.getElementById('ms-act-start').value, actualEndDate: document.getElementById('ms-act-end').value, owner: document.getElementById('ms-owner').value, remark: document.getElementById('ms-remark').value }; if(!item.phase) return alert('Activity Name required'); if(idx === '') milestones.push(item); else milestones[idx] = item; closeModal('modal-milestone'); renderMilestones(); }
        ...
        function wsUpdateDashPill() {
            const pill = document.getElementById('dashPill');
            if (!pill) return;
            const sel = document.getElementById('ws-select');
            if (sel && sel.value) {
                const opt = (sel.options && sel.selectedIndex >= 0) ? sel.options[sel.selectedIndex] : null;
                const name = opt ? opt.textContent : sel.value;
                pill.textContent = (name && name.trim()) ? name.trim() : sel.value;
            } else {
                pill.textContent = '—';
            }
        }
        window.wsUpdateDashPill = wsUpdateDashPill;

        function wsRefreshSelect() {
            const select = document.getElementById('ws-select');
            if (!select) { wsUpdateDashPill(); return; }
            const items = wsList();
            select.innerHTML = '';
            if (items.length === 0) {
                ...
                wsUpdateDashPill();
                try { window.twSyncWsSelectFromMain && window.twSyncWsSelectFromMain(); } catch(e) {}
                return;
            }
            items.forEach(w => {
                const opt = document.createElement('option');
        ...
        function wsExportCurrentExcel() {
            ...
            wsSetIndicator('Exported Excel ✓', true);
        }
        ...
        function wsClearLocalStorage() {
            ...
            wsEnsureDefault();
            wsRefreshSelect();
            wsSetIndicator('Storage cleared ✓', true);
        }
...
function gh6GetDashFromUrl() {
    try { return new URL(window.location.href).searchParams.get(GH6_PARAM); } catch(e){ return null; }
}
...
async function wsPersistRenameToGitHub(workspaceId, newName){
  ...
  try{
    ...
    wsSetIndicator('Renamed ✓ ' + new Date().toLocaleTimeString(), true);
  }catch(e){
    console.error(e);
...
async function wsSelectWorkspace(id, { saveCurrent=true, force=false } = {}) {
    if (!id) return;
    if (!force && id === wsCurrentId) return;
    ...
    // Keep header + selects in sync
    try { const sel=document.getElementById('ws-select'); if(sel) sel.value=id; } catch(e) {}
    try { window.wsUpdateDashPill && window.wsUpdateDashPill(); } catch(e) {}
    gh6SetDashInUrl(id);
    ...
    try {
        ...
        if (seq !== gh6LoadSeq || wsCurrentId !== id) return;

        try { window.wsCurrentMeta = data.meta || {}; } catch(e) {}
 // ignore stale response
        ...
        try {
            ...
            if (cached) {
                ...
                wsLastSerialized = wsStableSerializeState(wsGetState());
                wsSetIndicator('Loaded (cached) ✓', true);
            } else {
...
async function wsSaveNowToGitHub() {
    ...
    try {
        ...
        wsSetIndicator('Saved ✓ ' + new Date().toLocaleTimeString(), true);
...
async function wsInitWorkspaceManager() {
    ...
    const chosenId = (urlId && store.workspaces[urlId]) ? urlId : first;

    if (chosenId) {
        const sel = document.getElementById('ws-select');
        if (sel) sel.value = chosenId;
        // Force-load on refresh (do not rely on wsCurrentId pre-set)
        await wsSelectWorkspace(chosenId, { saveCurrent:false, force:true });
    } else {
...
<script>
  // ---------- Advanced Loader ----------
  const twLoader = {
    show(title='Loading…', sub='Please wait', pct=15) {
  ...
  async function twSelectWorkspace(id) {
    try {
      ...
      if (typeof wsSelectWorkspace === 'function') {
        twLoader.show('Loading dashboard…', 'Switching workspace', 35);
        twLoader.pct(45);
  ...
  async function twWsAction(action) {
    try {
      ...
      if (action === 'delete') {
        twLoader.show('Deleting dashboard…', 'Removing from GitHub / database', 40);
        return await Promise.resolve(wsDeleteCurrent && wsDeleteCurrent());
      }
      if (action === 'save') {
        twLoader.show('Saving dashboard…', 'Persisting to GitHub / database', 45);
  ...
  if (action === 'exportReport') {
    twLoader.show('Generating report…', 'Exporting JPG', 45);
    return await Promise.resolve(window.exportReport && window.exportReport());
  }
      if (action === 'import') {
  ...
  async function twInitApp() {
    try {
      twLoader.show('Starting…', 'Checking session', 15);
      ...
      twLoader.show('Loading dashboards…', 'Syncing workspace list', 45);
      ...
      twLoader.show('Almost done…', 'Finalizing UI', 88);
...
// Run-style Actions menu: reliable open/close + backdrop + outside click + Esc
(function(){
  ...
  async function perform(action){
    if(action==='new')         return window.wsOpenCreateModal?.();
    if(action==='rename')      return window.wsRenameCurrent?.();
    if(action==='save')        return window.wsSaveNow?.({prompt:true});
    if(action==='delete')      return window.wsDeleteCurrent?.();
    if(action==='export')      return window.wsExportCurrent?.();
    if(action==='exportExcel') return window.wsExportCurrentExcel?.();
    if(action==='exportReport') return window.exportReport?.();
    if(action==='import'){ const inp=qs('ws-import-file'); inp && inp.click(); return; }
    if(action==='publish')     return window.wsOpenPublishModal?.();
    if(action==='unpublish')   return window.wsUnpublishCurrent?.();
  }
  function bind(){
    const btn  = qs('ws-actions-btn');
    const menu = qs('ws-actions-menu');
    ...
    back && back.addEventListener('click', closeMenu);
    ...
    document.addEventListener('click',(e)=>{ if(!isOpen()) return; const inside=menu.contains(e.target)||btn.contains(e.target); if(!inside) closeMenu(); });
...
<div id="modal-publish" class="modal fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50">
  <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl animate-fade-in">
    <div class="flex justify-between items-center mb-4">
      ...
      <button onclick="closeModal('modal-publish')" class="text-slate-500 hover:text-slate-800" aria-label="Close">✕</button>
    </div>
...
(function(){
  ...
  async function doPublish(){
    try{
      ...
      try { window.wsCurrentMeta = Object.assign({}, window.wsCurrentMeta||{}, { published:true, visibility:'public', status:'published' }); } catch(e) {}
      try { window.wsUpdatePublishUI && window.wsUpdatePublishUI(window.wsCurrentMeta); } catch(e) {}
  ...
  async function doUnpublish(){
    try{
      ...
      try { window.wsCurrentMeta = Object.assign({}, window.wsCurrentMeta||{}, { published:false, visibility:'private', status:'private' }); } catch(e) {}
      try { window.wsUpdatePublishUI && window.wsUpdatePublishUI(window.wsCurrentMeta); } catch(e) {}
    } catch(e){
      alert(e.message || String(e));
    }
...
<script>
// Safety wiring: ensure Actions dropdown toggles
(function(){
  function qs(id){ return document.getElementById(id); }
  window.addEventListener('DOMContentLoaded', function(){
    ...
    const open = ()=>{ menu.classList.remove('hidden'); back.classList.remove('hidden'); btn.setAttribute('aria-expanded','true'); };
    const close= ()=>{ menu.classList.add('hidden'); back.classList.add('hidden'); btn.setAttribute('aria-expanded','false'); };
    // avoid double-binding: mark
    if(btn.dataset.twBound) return; btn.dataset.twBound='1';
    btn.addEventListener('click', (e)=>{ e.preventDefault(); e.stopPropagation(); menu.classList.contains('hidden') ? open() : close(); });
    back.addEventListener('click', close);
    document.addEventListener('click', (e)=>{ if(!menu.classList.contains('hidden')){ const inside = menu.contains(e.target) || btn.contains(e.target); if(!inside) close(); }});
    // close buttons inside menu
    menu.querySelectorAll('[data-ws-close]').forEach(x=>x.addEventListener('click', close));
  });
})();
</script>


<script id="ws-publish-ui">
  // Toggle Publish/Unpublish buttons + update pill based on current meta
  window.wsIsPublished = function(meta){
    const m = meta || window.wsCurrentMeta || {};
    return !!(
      m.published === true || m.isPublished === true || m.public === true ||
      m.visibility === 'public' || m.visibility === 'published' ||
      m.access === 'public' || m.access === 'published' ||
      m.status === 'published'
    );
  };

  window.wsUpdatePublishUI = function(meta){
    const published = window.wsIsPublished(meta);
    const pill = document.getElementById('ws-indicator');
    if (pill){
      pill.textContent = published ? 'Published' : 'Private';
      pill.className = published
        ? 'text-xs text-green-700 px-2 py-1 rounded-full bg-green-50 border border-green-200'
        : 'text-xs text-slate-600 px-2 py-1 rounded-full bg-slate-50 border border-slate-200';
    }
    const btnPub = document.getElementById('ws-action-publish') || document.querySelector('[data-ws-action="publish"]');
    const btnUnp = document.getElementById('ws-action-unpublish') || document.querySelector('[data-ws-action="unpublish"]');
    if (btnPub) btnPub.classList.toggle('hidden', published);
    if (btnUnp) btnUnp.classList.toggle('hidden', !published);
  };

  document.addEventListener('DOMContentLoaded', () => {
    try { window.wsUpdatePublishUI(window.wsCurrentMeta); } catch(e) {}
  });
</script>


<script>
/* === AUTHORITATIVE PUBLISH STATE FIX (FINAL) === */
(function () {
  function updatePublishIndicatorFromMeta() {
    const pill = document.getElementById('ws-indicator');
    if (!pill || !window.wsMeta) return;

    if (window.wsMeta.published === true) {
      ...
      pill.className =
        'text-xs px-2 py-1 rounded-full bg-slate-50 text-slate-600 border border-slate-200';
    }
  }

  // Hook into state-client load
  const _origApply = window.applyWorkspaceState;
  window.applyWorkspaceState = function (data) {
    if (data && data.meta) {
      window.wsMeta = data.meta;
    }
    if (typeof _origApply === 'function') {
      _origApply.apply(this, arguments);
    }
    updatePublishIndicatorFromMeta();
  };

  // Safety: run after DOM ready
  document.addEventListener('DOMContentLoaded', updatePublishIndicatorFromMeta);

  // Update immediately on publish/unpublish click
  document.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-ws-action]');
    if (!btn) return;
    const action = btn.getAttribute('data-ws-action');
    if (action === 'publish') {
      window.wsMeta = window.wsMeta || {};
      window.wsMeta.published = true;
      updatePublishIndicatorFromMeta();
    }
    if (action === 'unpublish') {
      window.wsMeta = window.wsMeta || {};
      window.wsMeta.published = false;
      updatePublishIndicatorFromMeta();
    }
  });
})();
