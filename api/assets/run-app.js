module.exports = async (req, res) => {
  ...
  function uploadFile(file){if(!file)return;toast('Reading file…','info');var reader=new FileReader();reader.onload=function(e){try{var data=new Uint8Array(e.target.result);var wb=XLSX.read(data,{type:'array'});var sheet=wb.Sheets[wb.SheetNames[0]];var rows=XLSX.utils.sheet_to_json(sheet);tickets=parseRows(rows);closeRunModal();generateAndSave();}catch(err){toast('Upload failed','error');}};reader.readAsArrayBuffer(file);} 

  async function adoSubmit(){toast('Querying ADO…','info');var org=(qs('adoOrg').value||'').trim();var project=(qs('adoProject').value||'').trim();var pat=(qs('adoPat').value||'').trim();var wiql=(qs('adoWiql').value||'').trim();if(!org||!project||!pat||!wiql){toast('Fill all ADO fields','error');return;}try{var resp=await fetch('/api/ado/run-tickets',{method:'POST',credentials:'same-origin',headers:{'Accept':'application/json','Content-Type':'application/json'},body:JSON.stringify({org:org,project:project,pat:pat,wiql:wiql})});var t=await resp.text();var j={};try{j=JSON.parse(t||'{}');}catch(e){j={error:t};}if(!resp.ok)throw new Error(j.error||j.message||t||('HTTP '+resp.status));var list=j.tickets||[];if(!Array.isArray(list)||!list.length){toast('No tickets returned','error');return;}tickets=list;closeAdoForm();closeRunModal();generateAndSave();}catch(e){toast('ADO sync failed','error');}}
  ...
  function init(){
    window.RunApp={ saveNow:function(){return saveToWorkspace();}, download:downloadJpg };
